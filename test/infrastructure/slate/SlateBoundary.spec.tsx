...
describe('Slate Boundary', () => {
  describe('traversing the slate data structure', () => {
    ...
    test.each([
      ...
      expect(pathFromXPath({ children: [xABTree] } as Editor, xPath)).toEqual(
        expectedPath
      )
    })
  })
  ...
  describe('BeschreibungsKomponente API', () => {
    ...
    const description = (content: Descendant[]): Element => {
      const result = {
        data_origin: 'text',
        region: 'TEI',
    ...
    it('Find Node By Wrapper ID', () => {
      ...
      expect(
        findSlateNodeByWrapperID(editor, data_origin, {
          children: [],
          id: '',
          label: '',
          level: 0,
    ...
    it('EinfuegeLogik Slate Node', async () => {
      ...
      insertSlateNode(editor, history, [0, 2, 0, 0, 0], jest.fn(), 'ID')

      const [node] = Editor.node(editor, [0, 2, 0, 0, 0]) as NodeEntry<Element>
    ...
    it('Find Node With Beschreibung', () => {
      ...
      expect(findSlateNodeAtPath(editor, [0, 1, 0, 0, 0])).toBeDefined()
    })
  ...
  describe('Node manipulation', () => {
    const children = [{ text: '' }]
    const a = { data_origin: 'a', children }
    const b = { data_origin: 'b', children }
    const a2 = { data_origin: 'a', children }
    const c = { data_origin: 'c', children }
    const d = { data_origin: 'd', children }
    const root = { data_origin: 'root', children: [a, b, a2] }
    const match = (data_origin: string) => (node: Descendant) =>
      Element.isElement(node) && node.data_origin === data_origin

    it('can replace children in element with one element', () => {
      const editor = createErfassungsEditor()
      render(
        <TestContext>
          <HSPEditor value={[root]} editor={editor} />
        </TestContext>
      )
      act(() => {
        replaceMatchingChildren(editor, root, match('a'), c)
      })
      waitFor(() => {
        const node = findSlateNodeAtPath(editor, [0])! as Element
        expect(node.children[0]).not.toMatchObject(a)
      })

      expect(findSlateNodeAtPath(editor, [0])).toMatchObject({
        children: [
          { data_origin: 'b', children },
          { data_origin: 'c', children },
        ],
      })
    })

    it('can replace children in element with multiple elements', () => {
      const editor = createErfassungsEditor()
      render(
        <TestContext>
          <HSPEditor value={[root]} editor={editor} />
      ...
      act(() => {
        replaceMatchingChildren(editor, root, match('b'), [c, d])
      })

      waitFor(() => {
        const node = findSlateNodeAtPath(editor, [0]) as Element
        expect(node.children[1]).not.toMatchObject(a)
      })

      expect(findSlateNodeAtPath(editor, [0])).toMatchObject({
        children: [
          { data_origin: 'a', children },
          { data_origin: 'a', children },
          { data_origin: 'c', children },
          { data_origin: 'd', children },
        ],
      })
    })
  })
